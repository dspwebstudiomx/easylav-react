    ...
    <Section className="dark:bg-dark py-12">
      <Container className="flex flex-col items-center justify-center w-full mx-auto ">
        ...
        >
          ...
          {/* Botones navegación */}
          <button
            className="hidden absolute left-0 top-1/2 -translate-y-1/2 p-0 sm:flex items-center justify-center w-14 h-14 text-center text-primary_light bg-primary hover:bg-primary_dark rounded-full shadow-lg hover:bg-gray-100 transition border border-gray-300 dark:bg-secondary dark:border-light dark:text-light"
            onClick={prevSlide}
            aria-label="Anterior">
            <MdArrowBack className="text-2xl" />
          </button>
          <button
            className="hidden absolute right-0 top-1/2 -translate-y-1/2 p-0 sm:flex items-center justify-center w-14 h-14 text-center text-primary_light bg-primary hover:bg-primary_dark rounded-full shadow-lg hover:bg-gray-100 transition border border-gray-300 dark:bg-secondary dark:border-light dark:text-light -mr-2"
            onClick={nextSlide}
        ...
        <div className="flex justify-center mt-12 gap-2">
          {sortedLocalServices.map((_, idx) => (
            <button
              key={idx}
              className={`w-4 h-4 rounded-full transition-all duration-300 ${idx === current ? 'bg-primary dark:bg-secondary_dark scale-90' : 'bg-primary_light border border-primary_dark dark:bg-secondary_light'}`}
